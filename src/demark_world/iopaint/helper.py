...
def concat_alpha_channel(rgb_np_img, alpha_channel) -> np.ndarray:
    if alpha_channel is not None:
        ...
        rgb_np_img = np.concatenate((rgb_np_img, alpha_channel[:, :, np.newaxis]), axis=-1)
    return rgb_np_img
...
def gen_frontend_mask(bgr_or_gray_mask):
    ...
    res_mask = np.zeros((bgr_or_gray_mask.shape[0], bgr_or_gray_mask.shape[1], 4), dtype=np.uint8)
