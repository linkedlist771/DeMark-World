...
def test_runway_sd_1_5_all_samplers(device):
    ...
    for sampler in all_samplers:
        ...
        if (
            sampler in [SDSampler.dpm2_karras, SDSampler.dpm2_a_karras, SDSampler.lms_karras]
            and device == "mps"
        ):
...
def test_runway_sd_1_5_cpu_offload(device, strategy, sampler):
    ...
    cfg = get_config(strategy=strategy, prompt="a fox sitting on a bench", sd_steps=sd_steps)
    cfg.sd_sampler = sampler
