...
class DDPM(torch.nn.Module):
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        if self.make_it_fit:
            n_params = len(
                [name for name, _ in itertools.chain(self.named_parameters(), self.named_buffers())]
            )
...
class LatentDiffusion(DDPM):
    ...
    def get_fold_unfold(
        ...
        if uf == 1 and df == 1:
            ...
            weighting = self.get_weighting(kernel_size[0], kernel_size[1], Ly, Lx, x.device).to(
                x.dtype
            )
    ...
    def progressive_denoising(
        ...
        if cond is not None:
            if isinstance(cond, dict):
                ...
                cond = (
                    [c[:batch_size] for c in cond] if isinstance(cond, list) else cond[:batch_size]
                )
