...
class StableDiffusionXLBrushNetPipeline(
    ...
    def __init__(
        ...
        # )
        add_watermarker = (
            add_watermarker if add_watermarker is not None else is_invisible_watermark_available()
        )
    ...
    def encode_prompt(
        ...
        tokenizers = (
            [self.tokenizer, self.tokenizer_2] if self.tokenizer is not None else [self.tokenizer_2]
        )
        text_encoders = (
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if isinstance(brushnet_keep[i], list):
                    cond_scale = [
                        c * s for c, s in zip(brushnet_conditioning_scale, brushnet_keep[i])
                    ]
                else:
                ...
                if guess_mode and self.do_classifier_free_guidance:
                    ...
                    up_block_res_samples = [
                        torch.cat([torch.zeros_like(d), d]) for d in up_block_res_samples
                    ]
