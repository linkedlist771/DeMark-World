...
class Config:
    def __init__(self) -> None:
        ...
        self.cxt = (
            self.lateral_channels_in_collection[1:][::-1][-self.cxt_num :] if self.cxt_num else []
        )
...
def build_backbone(bb_name, pretrained=True, params_settings=""):
    if bb_name == "vgg16":
        ...
        bb_net = list(
            vgg16_bn(pretrained=VGG16_BN_Weights.DEFAULT if pretrained else None).children()
        )[0]
        bb = nn.Sequential(
            OrderedDict(
                {
        ...
        bb_net = list(
            resnet50(pretrained=ResNet50_Weights.DEFAULT if pretrained else None).children()
        )
        bb = nn.Sequential(
            OrderedDict(
                {
...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.conv_out1 = nn.Sequential(
            nn.Conv2d(
                channels[3] // 2
                + ([N_dec_ipt, channels[3] // 8][ipt_cha_opt] if self.config.dec_ipt else 0),
                1,
                1,
                1,
                0,
    ...
    def get_patches_batch(self, x, p):
        ...
        for idx in range(x.shape[0]):
            ...
            for column_x in columns_x:
                patches_x += [
                    p.unsqueeze(0)
                    for p in torch.split(column_x, split_size_or_sections=_size_h, dim=-2)
                ]
